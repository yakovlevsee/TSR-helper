...
if not imguicheck or not sampevcheck or not encodingcheck or not lfscheck or not monetluacheck or not doesFileExist('moonloader/TSR Helper/Images/MedH_Images.png') then
	function main()
		...
		lua_thread.create(function()
			...
			while true do
				if progressbar and progressbar.max ~= nil and progressbar.downloadinglibname and progressbar.downloaded and progressbar.downloadingtheme then
					...
					renderFontDrawText(MedHfont, 'TSR Helper', x+ 5, y - 37, 0xFFFFFFFF)
		...
		sampAddChatMessage(('[TSR Helper]{EBEBEB} Началось скачивание необходимых файлов. Если скачивание не удастся, обратитесь к {3366FF}vk.com/user_j{ebebeb}.'),0xFF33F2)
...
local imgui_fm = imgui.OnFrame(
	...
	function(player)
		...
		if not IsPlayerConnected(fastmenuID) then
			...
			imgui.Begin(u8'Меню взаимодействия с игроками', windows.imgui_fm, imgui.WindowFlags.NoResize + imgui.WindowFlags.NoBringToFrontOnFocus + imgui.WindowFlags.NoCollapse + imgui.WindowFlags.NoScrollbar)
									...
									if imgui.Button(fa.ICON_FA_HAND_PAPER..u8' Поприветствовать', imgui.ImVec2(285,30)) then
										...
										if tonumber(os.date('%H', os.time(os.date('!*t')) + 2 * 60 * 60)) > 4 and tonumber(os.date('%H', os.time(os.date('!*t')) + 2 * 60 * 60)) < 13 then
											sendchatarray(configuration.main_settings.playcd, {
												{'Здравствуйте! Я являюсь {gender:сотрудником|сотрудницей} тюрьмы строго режима.', #configuration.main_settings.myname < 1 and gsub(sampGetPlayerNickname(select(2,sampGetPlayerIdByCharHandle(playerPed))), '_', ' ') or u8:decode(configuration.main_settings.myname), configuration.main_settings.replaceash and '{location: | | |}' or '{ | | | }'},
								...
								if imgui.Button(u8'Продолжить '..fa.ICON_FA_ARROW_RIGHT, imgui.ImVec2(285,30)) then
									if not inprocess then
										sendchatarray(configuration.main_settings.playcd, {
											...
											{'Сейчас я задам вам пару вопросов. Вы готовы ответить на них?'},
										})
										sobesetap[0] = 2
									else
										TsrHelperMessage('Не торопитесь, Вы уже отыгрываете что-то! Прервать отыгровку: {MC}page down{WC}')
								...
								if not inprocess then
									local reasons = {
										pass = {
											...
											['был в деморгане'] = {'К сожалению, я вынужден вам отказать во вступлении в нашу организацию, поскольку вы проходили лечение в психиатрическое больнице.', '/b Обновите медицинскую карту.'}
										},
										mc = {
							...
							if imgui.Button(u8'Повысить сотрудника '..fa.ICON_FA_ARROW_UP, imgui.ImVec2(285,40)) then
								if configuration.main_settings.myrankint >= 9 then
									...
									sendchatarray(configuration.main_settings.playcd, {
										...
										{'/todo Поздравляю с новыми погонами*нажав на кнопку "повысить должность".'},
							...
							if imgui.Button(u8'Выдать мут '..sampGetPlayerNickname(fastmenuID)..'['..fastmenuID..']', imgui.ImVec2(285,30)) then
								if configuration.main_settings.myrankint >= 9 then
									if #str(fmutebuff) > 0 then
										...
										{'/todo Молодцы что убрали этого выговор*нажав на кнопку "Снять выговор сотруднику"'},
										{'/do Изменения сохранены.'},
										{'/rmute %s %s %s', fastmenuID, u8:decode(fmuteint[0]), u8:decode(str(fmutebuff))},
											})
										else
						...
						if settingswindow[0] == 1 then
							...
							imgui.BeginGroup()
								...
								imgui.BeginGroup()
									...
									if imgui.ToggleButton(u8'Автоматическое открытие дверь и ворот', usersettings.autodoor) then
									...
									if imgui.Button(u8'Скачать '..fa.ICON_FA_ARROW_ALT_CIRCLE_DOWN) then
										local function DownloadFile(url, file)
											downloadUrlToFile(url,file,function(id,status)
												if status == dlstatus.STATUSEX_ENDDOWNLOAD then
													TsrHelperMessage('Обновление успешно загружено, скрипт перезагружается...')
												end
											end)
										end
...
function main()
	...
	sampRegisterChatCommand('invite', function(param)
		...
		return sendchatarray(configuration.main_settings.playcd, {
				...
				{'/me {gender:зашёл|зашла} в базу данных министерства обороны'},
				{'/me {gender:открыл|открыла} пункт "Сотрудники ТСР"'},
				{'/me {gender:внёс|внесла} все данные из документов призывника.'},
				{'/do Данные внесены.'},
				{'/invite %s', id},
		})
	end)
...
{'/do Планшет закреплён на грудном креплении.'},
	...
	sampRegisterChatCommand('fwarn', function(param)
		...
		local id,reason = param:match('(%d+) (.+)')
		if id == nil or reason == nil then
			return TsrHelperMessage('Используйте /fwarn [id] [причина].')
		end
		return sendchatarray(configuration.main_settings.playcd, {
				{'/do Планшет закреплён на грудном креплении.'},
				{'/me правой рукой {gender:снял|сняла} планшет с крепления'},
	...
	sampRegisterChatCommand('unfwarn', function(param)
		...
		local id = param:match('(%d+)')
		if id == nil then
			return TsrHelperMessage('Используйте /unfwarn [id].')
		end
		return sendchatarray(configuration.main_settings.playcd, {
										{'/do Планшет закреплён на грудном креплении.'},
										{'/me правой рукой {gender:снял|сняла} планшет с крепления.'},
										...
										{'/me {gender:зашёл|зашла} в пункт "Сотрудники ТСР".'},
										{'/me {gender:выбрал|выбрала} нужного сотрудника.'},
										{'/todo Благодарю за проделанную работу*нажав на кнопку "Снять выговор сотруднику"'},
										{'/do Изменения сохранены.'},
		})
	end)
